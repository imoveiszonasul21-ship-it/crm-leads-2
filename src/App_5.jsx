...
import GerenciaDiaria from "./GerenciaDiaria";
...
export default function App() {
  ...
  async function saveForm() {
    ...
    if (!form.corretor.trim()) faltando.push("Corretor responsável");
    if (isAdmin && !form.corretor_email?.trim()) faltando.push("E-mail do corretor");
    if (!form.imovel?.trim()) faltando.push("Imóvel de interesse");
    if (!form.valor?.trim()) faltando.push("Valor");
    if (!form.proximo_contato) faltando.push("Próximo contato agendado");
    if (!form.observacoes?.trim()) faltando.push("Observações");
      ...
      <div style={{ maxWidth: 1100, margin: "0 auto", padding: "20px" }}>
        <div style={{ display: "flex", gap: 6, marginBottom: 18, borderBottom: "1px solid #E5E7EB", flexWrap: "wrap" }}>
          {[
            ...
            ...(isAdmin
              ? [
                  { key: "meu_instagram", label: "Meu Instagram" },
                  { key: "gerencia", label: "Gerência diária" },
        ...
        {activeTab === "meu_instagram" && isAdmin && <AdminInstagram session={session} />}
        {activeTab === "gerencia" && isAdmin && <GerenciaDiaria />}
        {activeTab === "escrituras" && isAdmin && <Escrituras />}
        {activeTab === "agenda" && isAdmin && <Agenda />}
        ...
        <div style={overlayStyle}>
          <div style={modalStyle}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
              <Field label="Nome*"><input required style={inputStyle} value={form.nome} onChange={(e) => setForm({ ...form, nome: e.target.value })} placeholder="Nome completo do cliente" /></Field>
              <Field label="Telefone* (com DDD)"><input required style={inputStyle} value={form.telefone} onChange={(e) => setForm({ ...form, telefone: e.target.value })} placeholder="Ex: (21) 99999-9999" /></Field>
              <Field label="Corretor responsável (nome)*"><input required style={inputStyle} value={form.corretor} onChange={(e) => setForm({ ...form, corretor: e.target.value })} placeholder="Nome do corretor" /></Field>
              {isAdmin && (
              ...
              <Field label="Imóvel de interesse*"><input required style={inputStyle} value={form.imovel} onChange={(e) => setForm({ ...form, imovel: e.target.value })} placeholder="Ex: Apto 2q Jardim Europa" /></Field>
              <Field label="Valor* (R$)"><input required style={inputStyle} value={form.valor} onChange={(e) => setForm({ ...form, valor: e.target.value })} placeholder="Ex: 450000" /></Field>
              <Field label="Status*">
                <select style={inputStyle} value={form.status} onChange={(e) => setForm({ ...form, status: e.target.value })}>
                  {STATUSES.map((s) => <option key={s.key} value={s.key}>{s.label}</option>)}
                </select>
              </Field>
              <Field label="Próximo contato agendado*"><input required type="date" style={inputStyle} value={form.proximo_contato || ""} onChange={(e) => setForm({ ...form, proximo_contato: e.target.value })} /></Field>
              <div style={{ gridColumn: "1 / -1" }}>
                <Field label="Observações*"><textarea required style={{ ...inputStyle, minHeight: 70, resize: "vertical" }} value={form.observacoes || ""} onChange={(e) => setForm({ ...form, observacoes: e.target.value })} /></Field>
              </div>
            </div>

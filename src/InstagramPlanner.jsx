...
export default function InstagramPlanner({ session, isAdmin }) {
  ...
  function abrirDia(date) {
    ...
    setForm(
      existente || { corretor_email: corretorAtivo, data: iso, tipo: "feed", tema: "", legenda: "", status: "ideia" }
    );
    setModalDate(iso);
  }
                ...
                {post ? (
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ display: "flex", gap: 6, alignItems: "center", flexWrap: "wrap" }}>
                      <span style={{ fontSize: 11, fontWeight: 700, color: tipoInfo(post.tipo).color, background: tipoInfo(post.tipo).color + "1A", padding: "2px 8px", borderRadius: 20 }}>
                        {tipoInfo(post.tipo).label}
                      </span>
                      <span style={{ fontSize: 11, fontWeight: 700, color: statusInfoOf(post.status).color }}>
                        {statusInfoOf(post.status).label}
                      </span>
                    </div>

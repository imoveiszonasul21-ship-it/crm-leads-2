...
export default function AdminInstagram({ session }) {
  ...
  function abrirNovo(date) {
    setForm({ corretor_email: meuEmail, data: toISODate(date), tipo: "feed", tema: "", legenda: "", status: "ideia" });
    ...
    <div>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
        {diasDoMes.map((date) => {
          ...
          return (
            <div key={iso} style={{ background: "#fff", borderRadius: 10, border: "1px solid #E5E7EB", padding: "10px 14px", display: "flex", gap: 12, alignItems: "center" }}>
              ...
              <div style={{ flex: 1, display: "flex", gap: 6, flexWrap: "wrap", alignItems: "center" }}>
                {postsDoDia.map((post) => (
                  <span
                    ...
                    style={{ cursor: "pointer", fontSize: 12, fontWeight: 700, color: tipoInfo(post.tipo).color, background: tipoInfo(post.tipo).color + "1A", padding: "4px 10px", borderRadius: 20 }}
                  >
                    {tipoInfo(post.tipo).label}{post.tema ? ` · ${post.tema}` : ""}
                  </span>
                ))}
                <button onClick={() => abrirNovo(date)} style={{ fontSize: 12, fontWeight: 600, color: "#0F2438", background: "#F3F4F6", border: "none", borderRadius: 20, padding: "4px 10px" }}>
                  + post
                </button>
              </div>
            </div>
